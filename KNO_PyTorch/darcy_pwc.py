...
def main():
    ...
    x = torch.from_numpy(x_raw).float().view(1200, res_1d, res_1d, 1)
    y = torch.from_numpy(y_raw).float().view(1200, -1)

    x_grid_1d = torch.linspace(0, 1, res_1d)
    grid_y, grid_x = torch.meshgrid(x_grid_1d, x_grid_1d, indexing='ij')
    x_grid = torch.stack([grid_x, grid_y], dim=-1).float().to(device)
    ...
    torch.save(model.state_dict(), "saved_models/darcy_pwc_model.pt")
